    ...
    <el-scrollbar class="pb-10" wrap-class="hidden-scroll-x dialog-scroll-warp-height">
      <el-form ref="editForm" :rules="rules" :model="editModel" label-width="120px" class="dialog-form">
        <el-form-item label="数据字典名称" prop="name">
          <el-input v-model="editModel.name" placeholder="数据字典名称" />
        </el-form-item>
        <el-form-item label="数据字典编码" prop="code">
          <el-input v-model="editModel.code" placeholder="数据字典编码" />
        </el-form-item>
        ...
        <el-form-item label="数据字典备注" prop="remark">
  ...
  data() {
    return {
      ...
      rules: {
        ...
        status: [{ required: true, message: '数据字典状态不能为空' }],
        description: [{ max: 255, message: '数据字典描述不多于255个字符' }],
        remark: [{ max: 255, message: '数据字典备注不多于255个字符' }]
      }
    }
  },
  watch: {
