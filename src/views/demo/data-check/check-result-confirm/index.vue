<template>
  <div class="page-container">
    <el-divider content-position="left">校验结果确认</el-divider>
      ...
      <el-form ref="form1" :model="filter1" :rules="rules1" class="form-container" @submit.native.prevent>
        <el-row :gutter="10">
          ...
          <el-col :xs="24" :sm="12" :md="8" class="float-right">
            <el-button type="primary" icon="fa fa-sticky-note-o" circle title="一键确认" class="float-right ml-10" />
            <el-button type="success" icon="fa fa-sign-out" circle title="导出数据质量检验结果清单" class="float-right" />
            <el-button type="primary" icon="fa fa-search" circle title="查询" class="float-right" @click="handleFilter1" />
          </el-col>
        </el-row>
      </el-form>
    </div>
    <el-divider content-position="left">待确认清单</el-divider>
    <el-table ref="table1" v-loading="tableLoading1" :data="tableData1" row-key="id" stripe highlight-current-row class="width-100">
      ...
      <el-table-column label="操作" align="center" fixed="right" min-width="105">
        <template slot-scope="{row}">
          <el-button icon="fa fa-pencil-square-o" circle title="部门审核意见" @click="seeMore(row)" />
        </template>
      </el-table-column>
    </el-table>
