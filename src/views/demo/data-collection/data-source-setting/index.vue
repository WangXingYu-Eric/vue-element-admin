    ...
    <el-table ref="table" v-loading="tableLoading" :data="tableData" row-key="id" stripe highlight-current-row class="width-100">
      ...
      <el-table-column prop="password" label="密码" align="center" min-width="150">
        <template slot="header">
          <div class="cell">
            <span>密码</span>
            <i ref="faEye" :class="{'fa':true,'fa-eye-slash':true,'cursor-pointer':true,'ml-10':true}" @click="changePasswordVisible" />
          </div>
        </template>
        <template slot-scope="{row}">
          <template v-if="row.edit">
      ...
      <el-table-column prop="driver" label="驱动类" align="center" min-width="150">
        <template slot-scope="{row}">
          <template v-if="row.edit">
            <el-input v-model="row.driver" :placeholder="'请输入驱动类'" clearable size="mini" />
          </template>
      ...
      <el-table-column label="操作" align="center" fixed="right" min-width="100">
        <template slot-scope="{row}">
          <el-button v-if="row.edit" type="success" size="mini" icon="fa fa-check" circle title="保存" @click="confirmEdit(row)" />
          <el-button v-else type="primary" size="mini" icon="fa fa-edit" circle title="修改" @click="row.edit=!row.edit" />
          <el-button type="primary" size="mini" icon="fa fa-wrench" circle title="测试" @click.stop="()=>{}" />
        </template>
      </el-table-column>
    </el-table>
...
import { fetchList } from '@/api/demo/data-collection/data-source-setting'
export default {
