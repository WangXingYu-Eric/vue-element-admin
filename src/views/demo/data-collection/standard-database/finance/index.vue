    ...
    <el-row :gutter="10">
      <el-col :span="24">
        <el-table ref="table" v-loading="tableLoading" :data="tableData.filter(data => !search || data.tableName.toLowerCase().includes(search.toLowerCase() || data.tableComment.toLowerCase().includes(search.toLowerCase())))" row-key="id" stripe highlight-current-row class="width-100">
          ...
          <el-table-column prop="tableComment" label="描述" align="center" min-width="500">
            <template slot-scope="{row}">
              <complete-show :complete-str="row.tableComment?row.tableComment:'无'" :min-width="500" />
            </template>
          </el-table-column>
        </el-table>
      </el-col>
...
<script>
import { getTableList } from '@/api/demo/data-collection/standard-database/finance'
export default {
  name: 'FinanceIndex',
  data() {
    return {
